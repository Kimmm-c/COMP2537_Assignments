...
function get_pokedex(dex_num) {
    $.ajax({
        url: `https://blooming-sierra-96067.herokuapp.com/get_pokemons_by_dex?pokedex=${dex_num}`,
        type: "GET",
...
function get_pokemon_by_name(keyword) {
    $.ajax({
        url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_name?name=${keyword}`,
        type: "GET",
        success: process_pokemon
    })
}
...
function get_pokemons_habitat() {
    ...
    //console.log(habitat);
    $.ajax({
        url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_habitat?habitat=${habitat}`,
        type: "GET",
        success: process_pokemon
    })
...
function get_pokemons_type() {
    ...
    $.ajax({
        url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_type?type=${type}`,
        type: "GET",
        success: process_pokemon
    })
...
function process_pokemon(pokemon) {
    //console.log(pokemon);
    for (count = 0; count < pokemon.length; count++) {
        $("#pokemons_display").append(`<div class="pokemon">
    ${pokemon[count].id}    
    <img src=${pokemon[count].img}>
    <a href="profile.html" id=${pokemon[count].name} class="pokemon_name">${pokemon[count].name.toUpperCase()}</a>`)
    }
}
...
function retrieve_history() {
    ...
    if (key == "name") {
        $.ajax({
            url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_name?name=${val}`,
            type: "GET",
            success: process_pokemon
        })
    } else if (key == "habitat") {
        $.ajax({
            url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_habitat?habitat=${val}`,
            type: "GET",
            success: process_pokemon
        })
    } else if (key == "type") {
        $.ajax({
            url: `https://blooming-sierra-96067.herokuapp.com/get_pokemon_by_type?type=${val}`,
            type: "GET",
            success: process_pokemon
        })
    } else {
        $.ajax({
            url: `https://blooming-sierra-96067.herokuapp.com/get_pokemons_by_dex?pokedex=${val}`,
            type: "GET",
            success: process_pokemon
        })
    }
